...
function getMensaje(id,texto,titulo,tipo,tam){
	var msg = $$(id);
	msg.style.display = "block";
	var txt = "";
	var tit = $$(id+"_titulo");
	if(titulo){
		txt+="<span class=\"titulo_16_color\"><strong>"+titulo+"</strong></span><br><br>";
	}
	txt+=texto;
...
function enviar_sorteo(){
	...
	if(email=="" || email=="E-MAIL"){
		...
		if(!esEmail(email)){
			er+="- Tu <b>E-mail</b> debe ser correcto.<br/>";	
		}
	}
	if(er!=""){ 
		getMensaje("mensaje",er,"","error",200);	
	}else{
		...
		ajax.onreadystatechange = function(){
			if(ajax.readyState==4){
				...
				if(ajax.responseText!=""){
					...
					getMensaje("mensaje",ajax.responseText,"","error",200);
				}else{
